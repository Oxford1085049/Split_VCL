...
from models.Bayesian.multihead_bayesianmlp import MultiheadBayesianNN
from utils import compute_kl_divergence, create_coresets, set_seed
from data_creation.split_data import load_split_mnist



def continual_learning(model, train_datasets, coreset_datasets, test_datasets, tasks, epochs_per_task=5, batch_size=64, lr=0.001, nb_simul=10, vcl=False, reduction='sum', beta=1, beta_scheduler=None, compute_baseline=False, reuse_first=False):
    ...
    for task_id, (task_start, task_end) in enumerate(tasks):
        model.set_requires_grad(task_id)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        if reuse_first and task_id == 0:
            model.load_state_dict(torch.load(f'./outputs/SplitMNIST/model_{0}.pt'))
        ...
        if vcl:
            mu_previous, logvar_previous = mu_current.detach(), logvar_current.detach()
        # save the model
        torch.save(model.state_dict(), f'./outputs/SplitMNIST/models/model_{task_id}.pt')
...
def save_plots_to_pdf(overall_accuracies, task_accuracies, losses, overall_baseline, filename, compute_baseline):
    ...
    plt.savefig(os.path.join('outputs/SplitMNIST/plots', filename))
...
if __name__ == "__main__":
    ...
    if os.path.exists(f'./data/SplitMNIST/data.pkl') and not args.recompute_data:
        print('Loading from files. Use --recompute_data to force recompute.')
        with open(f'./data/SplitMNIST/data.pkl', 'rb') as f:
            train_datasets, test_datasets = pickle.load(f)
    else:
        train_datasets, test_datasets = load_split_mnist(root='./data', train_ratio=args.train_ratio)
        # check if the data folder exists
        if not os.path.exists('./data/SplitMNIST'):
            os.makedirs('./data/SplitMNIST')
        with open(f'./data/SplitMNIST/data.pkl', 'wb') as f:
            pickle.dump((train_datasets, test_datasets), f)
    
    if not os.path.exists('./outputs/SplitMNIST/logs'):
        os.makedirs('./outputs/SplitMNIST/logs')
    if not os.path.exists('./outputs/SplitMNIST/models'):
        os.makedirs('./outputs/SplitMNIST/models')
    if not os.path.exists('./outputs/SplitMNIST/plots'):
        os.makedirs('./outputs/SplitMNIST/plots')
    ...
    model_bayesian = MultiheadBayesianNN(28*28, args.hidden_size, 10, args.method, args.bias)
    overall_accuracies_bayesian, task_accuracies_bayesian, losses_bayesian, overall_baseline = continual_learning(model_bayesian, train_datasets, coreset_datasets, test_datasets, tasks, epochs_per_task=args.epochs, batch_size=args.batch_size, lr=args.lr, nb_simul=args.nb_simul, vcl=args.vcl, reduction=args.reduction, beta=args.beta, beta_scheduler=beta_scheduler, compute_baseline=args.compute_baseline, reuse_first=args.reuse_first)


    with open(f'./outputs/SplitMNIST/logs/{args.filename if args.filename else datetime.now().strftime("%Y%m%d_%H%M%S")}_Continual_Bayesian.pkl', 'wb') as f:
        pickle.dump((overall_accuracies_bayesian, task_accuracies_bayesian, losses_bayesian), f)
    
    save_plots_to_pdf(overall_accuracies_bayesian, task_accuracies_bayesian, losses_bayesian, overall_baseline, args.filename if args.filename else f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_Continual_Bayesian_MNIST.pdf", args.compute_baseline)
