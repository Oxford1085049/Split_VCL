...
def continual_learning(model, model2, train_datasets, coreset_datasets, test_datasets, tasks, epochs_per_task=5, batch_size=64, lr=0.001, nb_simul=10, vcl=False, reduction='sum', beta=1, beta_scheduler=None, compute_baseline=False, reuse_first=False, name=None):
    ...
    for task_id, (task_start, task_end) in enumerate(tasks):
                ...
                for _ in range(nb_simul):
                    ...
                    if vcl:
                        ...
                        if task_id >= 0: # not needed, was just to test without the first kl div with prior
                            ...
                            kl_losses[epoch] += 0.
                        if task_id == 1:
                            kl_loss2 = compute_kl_divergence(mu_current, logvar_current, mu_previous2, logvar_previous2, reduction)/len_train_loader
                            loss += kl_loss2
                            kl_losses2[epoch] += kl_loss2.item()
                            kl_losses2[epoch] += 0.
                            kl_loss0 = compute_kl_divergence(mu_current, logvar_current, mu_previous0, logvar_previous0, reduction)/len_train_loader
                            loss -= kl_loss0
                            kl_losses0[epoch] += kl_loss0.item()
                            kl_losses0[epoch] += 0.
        ...
        if task_id == 1:
            plt.plot(range(1, epochs_per_task+1), kl_losses2, label='KL Loss 2')
            plt.plot(range(1, epochs_per_task+1), kl_losses0, label='KL Loss 0')
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
